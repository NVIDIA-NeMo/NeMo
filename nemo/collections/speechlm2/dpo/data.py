...
def _validate(cut: Any, value: dict[str, Any]) -> None:
    ...
    if isinstance(prompt, str):
        ...
        prompt_ok = (
            isinstance(prompt.get("system", ""), str)
            and isinstance(prompt.get("user"), str)
            and bool(prompt["user"].strip())
        )
    else:
        prompt_ok = False
    if not prompt_ok:
...
class FiniteLhotsePreferenceCorpus:
    ...
    def load_rank_shards(self, rank: int) -> tuple[list[list[PreferencePair]], dict[str, Any]]:
        ...
        expected_rank_counts = rank_active_slots(pairs_per_update=self.pairs_per_update, world_size=self.world_size)
        ...
        return padded, {
            "cuts_path": str(self.path),
            "direct": True,
            "shuffle": False,
            "cycle": False,
            "pair_reads": reads,
            "exhausted_exactly": True,
            "active_pair_ids_sha256": _stable_hash(ids),
            "source_shards": self.source_shards,
            "pairs_per_update": self.pairs_per_update,
            "rank": rank,
            "rank_local_slots": len(padded[0]),
        }
