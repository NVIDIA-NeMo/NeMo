...
def _write_shards(
    *, checkpoint: Path, output: Path, specs: dict[str, TensorSpec], plan: list[list[str]]
    ...
    for index, names in enumerate(plan, start=1):
        ...
        destination = output / filename
        tensors = {name: torch.empty(specs[name].shape, dtype=specs[name].dtype, device="cpu") for name in names}
    ...
    *,
    ...
    try:
        ...
        (output / "model.safetensors.index.json").write_text(
            json.dumps(
                {
                    "metadata": {"total_size": sum(spec.payload_bytes for spec in candidate.values())},
                    "weight_map": weights,
                },
                indent=2,
                sort_keys=True,
        ...
        (output / "EVAL_MODEL_READY.json").write_text(
