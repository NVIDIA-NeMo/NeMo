...
def _dialog_turns(
    prompt: str | Mapping[str, str], completion: str, audio_tag: str
...
def _encode_completion(
    formatter: Any, *, prompt: str | Mapping[str, str], completion: str, audio_tag: str, audio_tag_id: int
) -> _EncodedCompletion:
    ...
    for style, turns, prefix_turns in (("slots", answered, empty_answer),):
        try:
            ...
            mask = (
                []
                if raw_mask is None
                else [bool(x) for x in torch.as_tensor(raw_mask).detach().cpu().reshape(-1).tolist()]
            )
            if len(mask) != len(input_ids) or not any(mask):
                ...
                shared = next(
                    (index for index, pair in enumerate(zip(input_ids, prefix_ids)) if pair[0] != pair[1]),
                    min(len(input_ids), len(prefix_ids)),
                )
...
class DPOSALMAutomodel(SALMAutomodel):
    ...
    def configure_model(self, *args: Any, **kwargs: Any) -> None:
        if self.cfg.get("init_from_checkpoint", None) is not None:
            raise RuntimeError(
                "DPO model construction must not load an experiment checkpoint before the strict source DCP"
            )
        ...
        if pair.pair_id not in self._encoded:
            ...
            self._encoded[pair.pair_id] = (
                _encode_completion(
                    ...
                    audio_tag=self.audio_locator_tag,
                    audio_tag_id=int(self.audio_locator_tag_id),
                ),
                _encode_completion(
                    formatter,
                    prompt=pair.prompt,
    ...
    def _completion_logprob(self, encoded: _EncodedCompletion, audio: torch.Tensor) -> torch.Tensor:
        ...
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16) if device.type == "cuda" else nullcontext():
            outputs = self(
                prepared["input_embeds"], attention_mask=prepared["attention_mask"], **prepared.get("llm_kwargs", {})
            )
    ...
    def _force_reshard(self) -> None:
        ...
        for _, module in self.named_modules():
            ...
            if (
                callable(get_state)
                and (group := getattr(get_state(), "_fsdp_param_group", None)) is not None
    ...
    def training_step(self, batch: PreferenceBatch, batch_idx: int):
        ...
        for pair, reference in zip(batch.pairs, references, strict=True):
            ...
            objective = dpo_pair_objective(
                ...
                chosen_reference_logp=ref_chosen,
                rejected_reference_logp=ref_rejected,
                beta=float(self.cfg.dpo.beta),
            )
        ...
        health = torch.tensor(
            [local_loss_sum, local_margin_sum, float(local_active)], device=self.device, dtype=torch.float64
        )
        if dist.is_initialized():
    ...
    def on_train_batch_end(self, outputs: Any, batch: PreferenceBatch, batch_idx: int) -> None:
        ...
        if self.global_rank == 0:
            ready = {
                "step": batch.global_step,
                "model_weights": str(destination / "model_weights.dcp"),
                "training_state": str(destination / "training_state.dcp"),
                "passed": True,
            }
    ...
    def on_train_end(self) -> None:
        ...
        if self.global_rank == 0:
            _write_json(
                self._output_root / "global_compact.json",
                {"status": "passed", "steps": self._metrics, "surface": self._surface.__dict__},
            )
