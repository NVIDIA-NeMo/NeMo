...
class StreamingDecodeState:
    ...
    gen_function: torch.Tensor | None
    ...
    # Present when either component selects vllm_omni. The session may own
    # Nemotron, EarTTS, or both.
...
class InferenceStepResult:
    ...
    asr_predicted_text_tokens: torch.Tensor | None
    predicted_text_strs: list[str]
    asr_predicted_text_strs: list[str] | None
    decoded_audio: torch.Tensor | None = None
    debug: dict | None = None
    predicted_function_tokens: torch.Tensor | None = None
    predicted_function_strs: list[str] | None = None
