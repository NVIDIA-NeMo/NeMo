...
class StreamingS2SPipeline(S2SPipelineInterface):
    """Streaming speech-to-speech pipeline. Single-stream only (``streaming.batch_size`` must be 1)."""
    ...
    def __init__(self, cfg: DictConfig, s2s_model: NemotronVoicechatInferenceWrapper):
        ...
        # ------------------------------------------------------------------
        # ``.get`` throughout rather than ``getattr``: the fallback below can be
        # a plain dict, and ``getattr`` would silently return the default for
        # keys that are actually present in it.
        self.streaming_cfg = cfg.get("streaming", {})
        self.input_sample_rate = self.streaming_cfg.get("input_sample_rate", 16000)
        self.output_sample_rate = self.streaming_cfg.get("output_sample_rate", 22050)
        self.batch_size = self.streaming_cfg.get("batch_size", 1)
        self.max_len = self.streaming_cfg.get("max_len", 8192)
        if self.batch_size != 1:
            raise ValueError(
                "StreamingS2SPipeline supports a single stream "
        ...
        # ------------------------------------------------------------------
        self.chunk_size_in_secs = self.streaming_cfg.get("chunk_size_in_secs", 0.08)
        ...
        self.buffer_size_in_secs = self.streaming_cfg.get("buffer_size_in_secs", 71 * 0.08)

        self.att_context_size = self.streaming_cfg.get("att_context_size", [70, 0])
        ...
        # ------------------------------------------------------------------
        s2s_cfg = cfg.get("s2s", {})
        self.system_prompt: str | None = s2s_cfg.get("system_prompt", None)
        if self.system_prompt:
            logging.info(
                f"System prompt configured: {self.system_prompt[:100]}{'...' if len(self.system_prompt) > 100 else ''}"
            )

        self._default_top_p: float | None = s2s_cfg.get("top_p", None)
        self._default_temperature: float | None = s2s_cfg.get("temperature", None)
        self._default_repetition_penalty: float | None = s2s_cfg.get("repetition_penalty", None)
        ...
        self.output_dir = cfg.get("output_dir", "./generated")

        # Parse and validate request type early, with a safe default
        req_type_cfg = self.streaming_cfg.get("request_type", "frame")
    ...
    def shutdown(self) -> None:
        """Release process-level resources owned by the model.

        Only the vLLM-Omni path has any -- engine subprocesses and a daemon
        thread -- but this is safe to call for every backend. Native engines
        no-op. Unlike the precision globals this is object-scoped, which is
        why it lives on the pipeline rather than the builder.

        Call from a ``finally`` after :meth:`run`, or from a server finalize.
        Idempotent.
        """
        self.s2s_model.shutdown()
    ...
    def output_capabilities(self):
        """Optional checkpoint heads and active-backend output availability."""
        return self.s2s_model.output_capabilities
    ...
    def generate_step_for_frames(self, frames: list[Frame], buffers: list[Tensor]) -> list[GenerateStepOutput]:
        ...
        if len(frames) != 1:
            raise ValueError("StreamingS2SPipeline supports a single stream (batch_size must be 1)")
        ...
        sampling_params = {
            ...
            if getattr(pipeline_state.options, k) is not None
        }
            ...
            if eos_flag:
                ...
                if ctx is not None:
                    ...
                    state.finalize_tokens(
                        ctx.gen_text,
                        ctx.gen_asr_text if self.output_capabilities.has_asr_head else None,
                        ctx.frame_idx,
                        tokenizer=tokenizer,
                        pad_id=pad_id,
                        gen_function=(
                            ctx.gen_function if self.output_capabilities.has_function_head else None
                        ),
                    )
                    timing_by_stream[stream_id] = ctx.timing

        # Close finished streams before reset_streams discards their decode
        # context: a vLLM session hangs off that context, and its consumer task
        # would leak until process exit. end_stream is idempotent, so any later
        # _abort_stream_request for the same stream is a no-op.
        for stream_id, eos_flag in zip(stream_ids, eos_flags):
            if eos_flag:
        ...
        for stream_id, eos_flag in zip(stream_ids, eos_flags):
            if eos_flag:
                ...
                logging.info(
                    ...
                    f"function: {state.output_function_text_str!r}"
                )
                ...
                compact_function = (state.raw_function_text or "").replace(pad_str, "·")
                ...
                logging.info(f"Stream {stream_id} function (with padding): {compact_function}")
        ...
        for idx, frame in enumerate(frames):
            ...
            function_text = result.predicted_function_strs[idx] if result.predicted_function_strs else ""

            state.append_step_output(audio, text=text, asr_text=asr_text, function_text=function_text)
    ...
    # Enough consecutive chunks that the autoregressive steps which trigger
    # torch.compile recompilation are all paid during warmup.
    ...
    def warmup(self, system_prompt: str | None = None, num_chunks: int | None = None) -> None:
        ...
        # Real requests may pass a file path, which decodes through librosa and
        # pays a one-time JIT on first use.  The silence frames below pass
        # samples, so they never reach it; warm it here on a throwaway file
        # instead, at double the rate so resampling is covered too.
        warmup_rate = self.input_sample_rate * 2
        with tempfile.NamedTemporaryFile(suffix=".wav") as warmup_audio:
            sf.write(warmup_audio.name, torch.zeros(warmup_rate // 2).numpy(), warmup_rate)
            read_audio(warmup_audio.name, target_sr=self.input_sample_rate)
    ...
    def generate_step(self, frames: list[Frame]) -> list[GenerateStepOutput]:
        ...
        for frame in empty_terminal_frames:
            # A prefill-only request may also be the stream's terminal
            # request (for example, a client disconnect immediately after
            # sequence_start). Tear down model/session state even though no
            # decode step ran.
            self._abort_stream_request(frame.stream_id)
            self.context_manager.reset_streams([frame.stream_id], [True])
            self.bufferer.rm_bufferer(frame.stream_id)

        if not non_empty_frames:
            # All frames are prefill-only — nothing to decode. Terminal
            # frames were closed above; callers retain the output accumulator
            # until they consume and delete it.
        ...
        # The audio buffer left-pads; strip that so the rest of the pipeline
        # sees unpadded samples.
        ...
        # Prefill-only frames get empty outputs so the returned list stays
    ...
    def _save_text_files(self, state: S2SStreamingOutput, base: str) -> None:
        """Save agent, ASR, and function text outputs to txt files.

        The agent channel is written even when empty, because an empty agent
        turn is a result; the optional channels are only written when the
        checkpoint produced them.
        """
        txt_dir = os.path.join(self.output_dir, "txt")
        os.makedirs(txt_dir, exist_ok=True)

        channels = (
            ("", state.output_text_str, True),
            ("_asr", state.output_asr_text_str, False),
            ("_function", state.output_function_text_str, False),
        )
        for suffix, text, write_when_empty in channels:
            if not isinstance(text, str) or not (text or write_when_empty):
                continue
            path = os.path.join(txt_dir, f"{base}{suffix}.txt")
            try:
                with open(path, "w", encoding="utf-8") as f:
                    f.write(text)
            except OSError:
                logging.warning(f"Failed to write {path}")
    ...
    def _save_ctm_files(self, state: S2SStreamingOutput, base: str) -> None:
        ...
        if state.token_function is not None:
            self._write_ctm(
                base,
                state.token_function[0, :total_frames],
                total_frames,
                total_samples,
                tokenizer,
                pad_id,
    ...
    def _prefill_system_prompt(
        self,
        stream_id: int,
        system_prompt: str | None = None,
        sampling_params: dict[str, float] | None = None,
    ) -> None:
        """Open a new stream on the model, system prompt included.

        Whether that means a native prompt prefill, a vLLM session, or both is
        the wrapper's business; the pipeline only announces that a stream is
        starting.
        ...
        Args:
            ...
            sampling_params: Per-stream text sampling parameters, for backends
                that fix sampling when the stream opens.
        """
        self.s2s_model.begin_stream(
            self.context_manager.get_context([stream_id]),
            system_prompt,
    ...
    def _abort_stream_request(self, stream_id: int) -> None:
        """Close a stream on the model. Idempotent."""
        self.s2s_model.end_stream(
            self.context_manager.get_context_for_stream(stream_id),
            request_id=self._request_id_for_stream(stream_id),
        )
