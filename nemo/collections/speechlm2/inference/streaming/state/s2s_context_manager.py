...
class S2SContextManager:
    ...
    ``infer_one_step()``.  One stream at a time (``batch_size`` must be 1).
    ...
    def __init__(
        ...
        self.device = s2s_model.device
        self.dtype = s2s_model.dtype
    ...
    def get_context(self, stream_ids: list[int]) -> StreamingDecodeState:
        ...
        if len(stream_ids) != 1:
            raise ValueError("S2SContextManager supports a single stream (batch_size must be 1)")
    ...
    def update_context(
        ...
        All cache and tensor mutations (llm_cache, tts_past_key_values,
        tts_code, perception_cache, tts_codec_cache, gen_text, gen_asr_text,
        etc.) are already applied in-place on the ``StreamingDecodeState`` by
