...
from nemo.collections.speechlm2.inference.model_wrappers.capabilities import AuxiliaryOutputCapabilities
...
class S2SStreamingOutput:
    ...
    capabilities: AuxiliaryOutputCapabilities | None = None
    ...
    _function_text_parts: list[str] = field(default_factory=list, repr=False)
    ...
    token_function: torch.Tensor | None = None
    ...
    raw_function_text: str | None = None
    ...
    @property
    def output_function_text_str(self) -> str:
        """Accumulated function-channel text. Joined lazily."""
    ...
    def reset(self) -> None:
        ...
        self._function_text_parts.clear()
    ...
    def append_step_output(
        ...
        if isinstance(function_text, str) and function_text:
            self._function_text_parts.append(function_text)
    ...
    def finalize_tokens(
        ...
        self.token_function = gen_function[:, :total_frames].clone().cpu() if gen_function is not None else None
        ...
        if self.token_asr_text is not None:
            ...
            self.asr_text_with_timestamps = None
            self.raw_asr_text = None
        if self.token_function is not None:
            self.function_text = _to_str(self.token_function)
            self.raw_function_text = _to_str(self.token_function, keep_pad=True)
        else:
            self.function_text = None
            self.raw_function_text = None
