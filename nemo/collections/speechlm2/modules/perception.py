...
class AudioPerceptionModule(NeuralModule, Exportable):
    ...
    def token_equivalent_duration(self) -> float:
        ...
        """
        frame_shift = (
            self.preprocessor.featurizer.hop_length
    ...
    def __init__(self, cfg: DictConfig):
        ...
        if isinstance(
            ...
            self.encoder_multilayer = ConformerMultiLayerFeatureExtractor(
                ...
                include_final_output=cfg.modality_adapter.get(
        ...
        self.rote = (
            self.from_config_dict(cfg.rote) if cfg.get("rote") is not None else None
        )
    ...
    def _apply_rote(self, encoder_emb, time_offset=None):
        ...
        if isinstance(encoder_emb, (list, tuple)):
            return type(encoder_emb)(
                self._apply_rote(emb, time_offset) for emb in encoder_emb
            )
        ...
        ) * self.encoder_frame_duration
        times = frame_times.unsqueeze(0).expand(b, -1)  # (B, T)
        if time_offset is not None:
            times = times + time_offset.to(
    ...
    def forward(
        ...
        encoded, encoded_len = self.modality_adapter(
            audio_signal=encoder_emb, length=encoded_len
        )
...
class IndependentDualEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.frame_shift_seconds <= 0:
            raise ValueError(
                f"frame_shift_seconds must be positive, got {frame_shift_seconds!r}."
            )
        for name, value in (
        ...
        self.d_model = self._encoder_width(asr_encoder) + self._encoder_width(
            auxiliary_encoder
        )
        ...
        if not all(
            ...
            raise TypeError(
                "IndependentDualEncoder requires two native sequence-packed encoders."
            )
    ...
    def _matching_positive_int(self, attr: str) -> int:
        values = [
            int(getattr(encoder, attr, -1) or -1)
            for encoder in (self.asr_encoder, self.auxiliary_encoder)
        ]
        if values[0] <= 0 or values[0] != values[1]:
            raise ValueError(
                f"Both encoders must have the same positive {attr}; got {values}."
            )
        return values[0]
    ...
    def _encoder_width(encoder: nn.Module) -> int:
        width = int(
            getattr(encoder, "_feat_out", getattr(encoder, "d_model", -1)) or -1
        )
        if width <= 0:
            raise ValueError(
                f"Could not determine output width for {type(encoder).__name__}."
            )
        return width
    ...
    def forward_sequence_packed(
        self, audio_signal, length=None
    ) -> PackedEncoderActivations:
        if not isinstance(audio_signal, PackedEncoderActivations):
            if length is None:
                raise ValueError(
                    "length is required for padded IndependentDualEncoder input."
                )
            audio_signal = pack_encoder_output(audio_signal.transpose(1, 2), length)
        elif length is not None and not torch.equal(
        ...
        asr = self._forward_branch(
            self.asr_encoder, audio_signal, self.asr_chunk_size_seconds
        )
        grad_context = (
            torch.no_grad() if self.freeze_auxiliary else contextlib.nullcontext()
        )
        with grad_context:
...
class QformerConnector(nn.Module):
    def __init__(
        ...
        self.layer_prompts = nn.ParameterList(
            [
                nn.Parameter(
                    torch.randn(1, self.prompt_size, self.encoder_config.d_model)
                )
                for _ in range(num_inputs)
            ]
        )
        self.layer_weights = nn.Parameter(
            torch.zeros(self.prompt_size, num_inputs, dtype=torch.float)
        )
        ...
        if hasattr(
            qformer_config, "_attn_implementation"
        ...
        self.proj = nn.Sequential(
            ...
            nn.Linear(
                self.encoder_config.d_model, self.llm_config.hidden_size
    ...
    def forward(self, audio_signal: list[torch.Tensor], length):
        ...
        assert len(audio_signal) == expected_num, (
        ...
        for idx, encoder_hidden_state in enumerate(audio_signal):
            layer_prompt = self.layer_prompts[idx].expand(
                encoder_hidden_state.size(0), -1, -1
            )
        ...
        norm_weights = torch.nn.functional.softmax(
            self.layer_weights, dim=-1
        ).unsqueeze(-1)
        output = (layer_prompt_outputs * norm_weights).sum(
            dim=2
        )  # (b, prompt_size, d_llm)
        ...
        return output, torch.tensor(
            [output.shape[1]] * output.shape[0], device=output.device, dtype=torch.long
        )
