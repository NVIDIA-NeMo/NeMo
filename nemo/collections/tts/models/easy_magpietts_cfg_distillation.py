...
def _validate_configuration(cfg: DictConfig) -> None:
    ...
    if hasattr(cfg, "backbone_loss_weight") and not (cfg.get("backbone_loss_weight") >= 0.0):
        raise ValueError("`backbone_loss_weight` must be non negative")
...
class _AudioCodesStep:
    ...
    codes_t_backbone: Tensor
    codes_t_argmax_backbone: Tensor
    logits_t_backbone: Tensor
    ...
    def unstack(
        ...
        self.codes_t_backbone = self.codes_t_backbone.view(batch_size, num_codebooks, fs_factor)
        self.codes_t_argmax_backbone = self.codes_t_argmax_backbone.view(batch_size, num_codebooks, fs_factor)
...
class _StreamingState:
    ...
    pred_codes_backbone: list[Tensor]
    pred_codes_logits_backbone: list[Tensor]
    ...
    last_audio_codes_backbone: Optional[Tensor]
    ...
    def create(
        ...
        obj = _StreamingState(
            ...
            last_audio_codes_backbone=None,
    ...
    def update_last_audio_codes(
        ...
        if self.last_audio_codes_backbone is None:
            self.last_audio_codes_backbone = audio_codes.codes_t_backbone
        else:
            update_mask = needs_audio.view(self.batch_size, 1).expand_as(audio_codes.codes_t_backbone)
            self.last_audio_codes_backbone = torch.where(
                update_mask, audio_codes.codes_t_backbone, self.last_audio_codes_backbone
            )
    ...
    def update_audio_end_status(
        ...
        # Use LT codes for EOS prediction policy.
    ...
    def add_audio_codes(
        ...
        self.pred_codes_backbone.append(audio_codes.codes_t_backbone)
        self.pred_codes_logits_backbone.append(audio_codes.logits_t_backbone.unsqueeze(1))
...
class _TeacherInferenceEngine:
    ...
    def _prepare_input_audio_phase(
        ...
        if status.audio.any():
            ...
            # Use LT codes for rollout policy.
    ...
    def _predict_audio_codes(self, state: _StreamingState) -> _AudioCodesStep:
        ...
        codes_t_argmax_backbone = (
            codes_t_backbone
            if temp <= 0.0
            else self.model.sample_codes_from_logits(logits_t_backbone, temperature=0.01)
        )

        codes_t, logits_t = _lt_sample_autoregressive(
            model=self.model,
        ...
        return _AudioCodesStep(
            ...
            codes_t_argmax_backbone=codes_t_argmax_backbone,
            logits_t_backbone=logits_t_backbone,
            codes_t=codes_t,
    ...
    def _process_predictions(
        ...
        if status.audio.any():
            ...
            audio_codes = self._predict_audio_codes(state)
    ...
    def _finalize_predicted_codes(
        ...
        with torch.no_grad():
            codes_backbone = torch.cat(state.pred_codes_backbone, dim=-1)
            logits_backbone = torch.cat(state.pred_codes_logits_backbone, dim=1)
            ...
            pred_codes_backbone = torch.zeros(*codes_size, dtype=codes_backbone.dtype, device=device)
            pred_logits_backbone = torch.zeros(*logits_size, dtype=logits_backbone.dtype, device=device)
            ...
            for i in range(bs):
                ...
                pred_codes_backbone[i, :, :length] = codes_backbone[i, :, start:end]
                pred_logits_backbone[i, : end_ - start_, :] = logits_backbone[i, start_:end_, :]
                pred_codes[i, :, :length] = codes[i, :, start:end]
                pred_logits[i, : end_ - start_, :] = logits[i, start_:end_, :]

        return _TeacherPredictedCodes(
            codes_backbone=pred_codes_backbone,
            logits_backbone=pred_logits_backbone,
    ...
    def infer_batch(
        ...
        """TBD."""
        if self.model.local_transformer_type != LocalTransformerType.AR:
            raise ValueError(
                f"Only `LocalTransformerType.AR` is supported, but got `{self.model.local_transformer_type}`."
            )
        ...
        with torch.no_grad():
            ...
            if teacher_codes.codes.size(-1) != teacher_codes.codes_backbone.size(-1):
                raise RuntimeError(
                    "Mismatch between backbone and local-transformer generated code lengths: "
                    f"`teacher_codes.codes_backbone` has length {teacher_codes.codes_backbone.size(-1)}, "
                    f"while `teacher_codes.codes` has length {teacher_codes.codes.size(-1)}."
                )
            teacher_output = _TeacherOutput(
                codes_backbone=teacher_codes.codes_backbone,
                logits_backbone=teacher_codes.logits_backbone,
...
_MONITORED_KEYS_TRAIN: list[str] = [
    ...
    _get_loss_key(key=_LossKey.kl_loss, mode=_LossMode.backbone),
    _get_loss_key(key=_LossKey.ce_loss, mode=_LossMode.backbone),
    _get_loss_key(key=_LossKey.nrmse_loss, mode=_LossMode.backbone),
...
class EasyMagpieCFGDistillation(EasyMagpieTTSModel):
    ...
    def _process_batch(
        ...
        if self.local_transformer_type != LocalTransformerType.AR:
            raise ValueError(f"Only `LocalTransformerType.AR` is supported, but got `{self.local_transformer_type}`.")
        logits_phonemes = None
        batch_combined, batch_delay = self._prepare_student_input(batch, mode, training_mode)
    ...
    def _process_batch_distillation(
        ...
        student_output = self._process_batch(batch, mode)
        ...
        output = self._compute_loss(batch, teacher_output, student_output)
